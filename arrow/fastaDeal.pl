...
=head1 Contact & Version
  ...
  Version: 5,  Date: 20017-3-4

=head1 Command-line Option
 
  --attribute <str>       get differnt attributions, id:head:seq:len:lenwogap:gc:x:lc:uc:n.   
  ...
  --substruct <str|file>  chr1-100-200(or file),get a fragment from the sequence based on start(1st:1)/end coordinates
  ...
  --reform <str>          lowerize|upperize|line50|pure|link1000/splitn, reform/modify the sequence as you want
...
=head1 Usage Exmples
  ...
  fastaDeal.pl -sub chr1-100-200 chr1.fa
  fastaDeal.pl -sub chr1-100-200 -rev -com chr1.fa

  fastaDeal.pl -reform upperize  cds.fa
  fastaDeal.pl -reform link1000  cds.fa
  fastaDeal.pl -reform splitn  cds.fa
...
cut_fasta($ARGV[0],$File_in_dir_num) if($Cuts || $Cutf);
...
sub reform_seq {
	...
	$/=">";<>;$/="\n";
	my $linkseq;
	my $linkn = 'N' x $1 if ($Reform =~ /link(\d+)/i);
	while (<>) {
		my $title = $_;
		...
		if ($Reform eq "splitn"){
			...
			for ($i=0;$i<length($seq);$i++){
				if ($seqs[$i] eq 'N' or $seqs[$i] eq 'n'){
					if ($lable){
						print ">${seq_name}_${start}_$i\n$subseq\n";
						$lable = 0;
						$subseq = ''
					}
					$start = $i + 2;
...
sub substruct_frag{
	my (%seq,$seq_name,$head,$seq);
	
	$/=">"; <>; $/="\n";
	while (<>) {
		chomp;
		$seq_name = $_;
		$seq_name = $1 if($seq_name =~ /^(\S+)/);
	...
	if (-e $Substruct){
		...
		while (<IN>){
			...
			next if (/^#/);
			my @s = split;
			next if (not exists $seq{$s[0]});
			subSeq($s[0],$seq{$s[0]},$s[1],$s[2]);
		}
		close IN;
	}elsif($Substruct =~ /(\S+)-(start|\d+)-(end|\d+)/){
		my ($chr,$sub_start,$sub_end) = ($1,$2,$3);
		subSeq($chr,$seq{$chr},$sub_start,$sub_end)
	}else{
		die "Error,please use \"chr-start-end\"";
#			subSeq($seq_name,$seq,'start','end')
	}
}

sub subSeq{
	my ($seq_name,$seq,$sub_start,$sub_end) = @_;
    my $sub_seq;
	my $seq_len = length($seq);
	my $sub_head = ">".$seq_name;
    ($sub_start,$sub_end) = ($sub_end,$sub_start) if ($sub_start > $sub_end);
...
sub cut_fasta{
	...
	if ($Cutf) {
		$file_num = $Cutf;
		$seq_num = ($total_num % $file_num == 0) ? ($total_num / $file_num) : (int($total_num / $file_num) + 1);
	}
