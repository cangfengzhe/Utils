...
label_colors = [
    [i[0] / 255.0, i[1] / 255.0, i[2] / 255.0, 1]
...
def venn4(labels, names=['A', 'B', 'C', 'D'], **options):
    ...
    # legend
    draw_text(fig, ax, 0.13, 0.18, names[0], label_colors[0], fontsize=fontsize)
    draw_text(fig, ax, 0.18, 0.83, names[1], label_colors[1], fontsize=fontsize)
    draw_text(fig, ax, 0.82, 0.83, names[2], label_colors[2], fontsize=fontsize)
    draw_text(fig, ax, 0.87, 0.18, names[3], label_colors[3], fontsize=fontsize)
    leg = ax.legend(names, loc='best', fancybox=True)
    leg.get_frame().set_alpha(0.5)
